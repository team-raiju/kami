...
@Component({
  selector: "bot-3d",
  template: `<canvas #botCanvas id="bot-canvas" class="!w-full !h-full"></canvas>`,
})
export class Bot3DComponent {
  @ViewChild("botCanvas", { static: true })
  ...
  async loadScene() {
    ...
    const model = await loader.loadAsync(`models/${this.name()}.glb`);
