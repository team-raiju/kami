...
Uma das grandes dificuldades do search é garantir que as amostragem de paredes, sempre serão realizadas no mesmo ponto. Isto é, no início da celula (na direção vertical) e bem no meio na (posição horizontal). Porque caso a posição de amostragem mude, o valor dos sensores pode mudar de tal forma a ter falsos positivos ou negativos na leitura da parede. Para isso a calibração do movimento deve estar bem boa. Além disso a leitura dos sensores deve ser estável e não ser muito influenciada por fatores externos.
